...
function createFormContent($_label, $_type, $_name, $_place_holder = '', $_is_required = true, $_default_value = '', $_autofocus = '')
{
	...
	switch ($_type) {
		case 'textarea':
			echo '<textarea class="form-control" name="'.$_name.'" rows="3" placeholder="'.$_place_holder.'" value="'.$_value.'"required>'.$_value.'</textarea>';
			break;
...
function createUploadArea($_label, $_name, $labelUpload = 'Select Image - maximum file size is 1 MB', $_edit = false)
{
...
function createSelect($_label, $_name, $_option, $_attr = '', $_default_value = '')
{
	...
	echo '<select class="form-control '.$_attr.'" name="'.$_name.'" required>';
...
function createFormButton($_label, $_type, $_name, $_opt_class = '')
{
    ...
    echo '<button type="'.$_type.'" name="'.$_name.'" class="btn btn-primary float-left'.$_opt_class.'" style="margin-top: 5px;">'.$_label.'</button>';
    echo '</div>';
}
